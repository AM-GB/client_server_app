...
def create_presence_message(account_name, CONFIGS):
    ...
    CLIENT_LOGGER.info('Формирование сообщение для отправки на сервер')
    return message


def handle_response(message, CONFIGS):
    CLIENT_LOGGER.info('Обработка собщения от сервера')
    if CONFIGS.get('RESPONSE') in message:
        if message[CONFIGS.get('RESPONSE')] == 200:
            CLIENT_LOGGER.info('Успешная обработка сообщения')
            return '200 : OK'
        CLIENT_LOGGER.error('Невнрное сообщение, обработка невозможно')
...
def main():
    ...
    try:
        ...
        CLIENT_LOGGER.info(hanlded_response)
