...
class Tests(unittest.TestCase):
    ...
    test_success_receive = {CONFIGS['PRESENCE']: 200}
    test_error_receive = {
        CONFIGS['PRESENCE']: 400,
    ...
    def test_get_message(self):
        test_socket_ok = TestSocket(self.test_success_receive)
        test_socket_err = TestSocket(self.test_error_receive)
        self.assertEqual(get_message(
            test_socket_ok, self.CONFIGS), self.test_success_receive)
        self.assertEqual(get_message(
            test_socket_err, self.CONFIGS), self.test_error_receive)
