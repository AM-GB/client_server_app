    ...
    while True:
        ...
        if recv_data_lst:
            for client_with_message in recv_data_lst:
                try:
                    msg_client = get_message(client_with_message, CONFIGS)
                    print(msg_client)
                    handle_message(msg_client, messages,
                                   client_with_message, CONFIGS)
                except:
                    SERVER_LOGGER.info(
        ...
        if messages and send_data_lst:
            ...
            message = {
                ...
                CONFIGS['SENDER']: messages[0],
                CONFIGS['TIME']: time.time(),
                CONFIGS['MESSAGE_TEXT']: messages[0]
            }
            del messages[0]
